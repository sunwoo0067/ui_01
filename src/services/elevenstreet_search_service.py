...
class ElevenStreetSearchService:
    """11번가 상품 검색 서비스"""
    ...
    def __init__(self):
        ...
        # 11번가 검색 URL
        self.search_base_url = "https://search.11st.co.kr/Search.tmall"
    ...
    async def search_products(self, keyword: str, 
                            ...
                            max_price: Optional[int] = None) -> List[ElevenStreetProduct]:
        """
        11번가 상품 검색
        ...
        Returns:
            List[ElevenStreetProduct]: 검색된 상품 목록
        """
        try:
            logger.info(f"11번가 상품 검색 시작: {keyword}, 페이지: {page}")
            ...
            if html:
                ...
                logger.info(f"11번가 상품 검색 완료: {len(products)}개 상품")
                return products
            else:
                logger.error(f"11번가 검색 요청 실패")
                return []
                        
        except Exception as e:
            self.error_handler.log_error(e, {
                'operation': "11번가 상품 검색 실패",
                'keyword': keyword,
                'page': page
            })
            return []
    ...
    async def get_product_details(self, product_url: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            logger.info(f"11번가 상품 상세 정보 조회: {product_url}")
            
            html = await self.scraper.get_page_content(product_url)
            if html:
                details = await self._parse_product_details(html)
                logger.info(f"11번가 상품 상세 정보 조회 완료")
                return details
            else:
                logger.error(f"11번가 상품 상세 조회 실패")
                return None
                        
        except Exception as e:
            self.error_handler.log_error(e, {
                'operation': "11번가 상품 상세 조회 실패",
                'product_url': product_url
            })
            return None

    async def _parse_search_results(self, html: str, keyword: str) -> List[ElevenStreetProduct]:
        """검색 결과 파싱"""
        products = []
        ...
        try:
            ...
            # 11번가 상품 리스트 선택자 (실제 구조에 따라 조정 필요)
            product_items = soup.select('.c_card_item, .item, .product_item')
            
            for item in product_items:
                try:
                    # 상품명
                    name_elem = item.select_one('.c_card_item_name, .item_name, .product_name')
                    if not name_elem:
                        continue
                    name = name_elem.get_text(strip=True)
                    
                    # 가격
                    price_elem = item.select_one('.c_card_item_price, .item_price, .product_price')
                    if not price_elem:
                        continue
                    ...
                    # 원가 (할인가가 있는 경우)
                    original_price_elem = item.select_one('.c_card_item_original_price, .item_original_price')
                    original_price = None
                    ...
                    # 할인율
                    discount_elem = item.select_one('.c_card_item_discount, .item_discount')
                    discount_rate = None
                    if discount_elem:
                    ...
                    # 판매자
                    seller_elem = item.select_one('.c_card_item_seller, .item_seller, .product_seller')
                    seller = seller_elem.get_text(strip=True) if seller_elem else "11번가"
                    ...
                    if link_elem:
                        ...
                        if href:
                            product_url = urljoin("https://www.11st.co.kr", href)
                    ...
                    if img_elem:
                        ...
                        if image_url:
                            image_url = urljoin("https://www.11st.co.kr", image_url)
                    
                    # 평점
                    rating_elem = item.select_one('.c_card_item_rating, .item_rating, .product_rating')
                    rating = None
                    if rating_elem:
                    ...
                    # 리뷰 수
                    review_elem = item.select_one('.c_card_item_review_count, .item_review_count')
                    review_count = None
                    if review_elem:
                    ...
                    # 배송 정보
                    shipping_elem = item.select_one('.c_card_item_shipping, .item_shipping')
                    shipping_info = None
                    if shipping_elem:
                        shipping_info = shipping_elem.get_text(strip=True)
                    
                    product = ElevenStreetProduct(
                        name=name,
                        price=price,
            ...
            logger.info(f"11번가 검색 결과 파싱 완료: {len(products)}개 상품")
            return products
            
        except Exception as e:
            logger.error(f"11번가 검색 결과 파싱 실패: {e}")
            return []
    ...
    async def _parse_product_details(self, html: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for img in img_elements:
                ...
                if src:
                    images.append(urljoin("https://www.11st.co.kr", src))
            ...
            logger.error(f"11번가 상품 상세 정보 파싱 실패: {e}")
            return {}
    ...
    async def save_products_to_database(self, products: List[ElevenStreetProduct], keyword: str) -> bool:
        """상품 정보를 데이터베이스에 저장"""
        try:
            logger.info(f"11번가 상품 {len(products)}개를 데이터베이스에 저장")
            ...
            logger.info(f"✅ 11번가 상품 저장 완료: {len(products)}개")
            return True
            
        except Exception as e:
            self.error_handler.log_error(e, {
                'operation': "11번가 상품 데이터베이스 저장 실패",
                'keyword': keyword,
