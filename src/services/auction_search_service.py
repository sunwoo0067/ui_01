...
class AuctionSearchService:
    ...
    def __init__(self):
        ...
        # 옥션 검색 URL
        self.search_base_url = "https://browse.auction.co.kr/search"
    ...
    async def search_products(self, keyword: str, 
                            ...
                            max_price: Optional[int] = None) -> List[AuctionProduct]:
        """
        옥션 상품 검색
        ...
        Returns:
            List[AuctionProduct]: 검색된 상품 목록
        """
        try:
            logger.info(f"옥션 상품 검색 시작: {keyword}, 페이지: {page}")
            ...
            if html:
                ...
                logger.info(f"옥션 상품 검색 완료: {len(products)}개 상품")
                return products
            else:
                logger.error(f"옥션 검색 요청 실패")
                return []
                        
        except Exception as e:
            self.error_handler.log_error(e, {
                'operation': "옥션 상품 검색 실패",
                'keyword': keyword,
                'page': page
            })
            return []
    ...
    async def get_product_details(self, product_url: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            logger.info(f"옥션 상품 상세 정보 조회: {product_url}")
            
            html = await self.scraper.get_page_content(product_url)
            if html:
                details = await self._parse_product_details(html)
                logger.info(f"옥션 상품 상세 정보 조회 완료")
                return details
            else:
                logger.error(f"옥션 상품 상세 조회 실패")
                return None
                        
        except Exception as e:
            self.error_handler.log_error(e, {
                'operation': "옥션 상품 상세 조회 실패",
                'product_url': product_url
            })
            return None

    async def _parse_search_results(self, html: str, keyword: str) -> List[AuctionProduct]:
        """검색 결과 파싱"""
        products = []
        ...
        try:
            ...
            for item in product_items:
                try:
                    ...
                    seller = seller_elem.get_text(strip=True) if seller_elem else "옥션"
                    ...
                    if link_elem:
                        ...
                        if href:
                            product_url = urljoin("https://www.auction.co.kr", href)
                    ...
                    if img_elem:
                        ...
                        if image_url:
                            image_url = urljoin("https://www.auction.co.kr", image_url)
            ...
            logger.info(f"옥션 검색 결과 파싱 완료: {len(products)}개 상품")
            return products
            
        except Exception as e:
            logger.error(f"옥션 검색 결과 파싱 실패: {e}")
            return []
    ...
    async def _parse_product_details(self, html: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for img in img_elements:
                ...
                if src:
                    images.append(urljoin("https://www.auction.co.kr", src))
            ...
            logger.error(f"옥션 상품 상세 정보 파싱 실패: {e}")
            return {}
    ...
    async def save_products_to_database(self, products: List[AuctionProduct], keyword: str) -> bool:
        """상품 정보를 데이터베이스에 저장"""
        try:
            logger.info(f"옥션 상품 {len(products)}개를 데이터베이스에 저장")
            ...
            logger.info(f"✅ 옥션 상품 저장 완료: {len(products)}개")
            return True
            
        except Exception as e:
            self.error_handler.log_error(e, {
                'operation': "옥션 상품 데이터베이스 저장 실패",
                'keyword': keyword,
